...
public class CycloneToSpdxTest {
	...
	static final Path CDX_TEST_RESOURCES = Paths.get("src", "test", "resources", "specification", "tools",
			"src", "test", "resources");
	static final Path V23_BOM_PATH = Paths.get("src", "test", "resources", "bom.json");
	...
	public void testValidSbomV1dot4() throws CycloneConversionException, IOException, InvalidSPDXAnalysisException {
		...
		try {
			...
			String created = spdxDoc.getCreationInfo().getCreated();
			assertTrue(created.startsWith("2020-04-13T20:20:39"));
    ...
    @Test
    public void testv23() throws ParseException, CycloneConversionException, InvalidSPDXAnalysisException {
    	final File v23Bom = V23_BOM_PATH.toFile();
    	final Bom cycloneBom = BomParserFactory.createParser(v23Bom).parse(v23Bom);
    	final IModelStore store = new InMemSpdxStore();
		CycloneSpdxConverter converter = new CycloneSpdxConverter(cycloneBom, store);
		converter.convert();
		final SpdxDocument doc = SpdxModelFactory.createSpdxDocument(store, converter.getDocumentUri(), new ModelCopyManager());
